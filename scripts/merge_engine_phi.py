...
# ── φ-harmonic jitter constants ────────────────────────────────────────────
PHI = (1.0 + math.sqrt(5.0)) / 2.0
GAMMA_JITTER = PHI ** -8          # ≈ 0.0212862 — pulse-envelope damping
T_MAX_SOAK = 10.0
DELTA_THETA = 0.018
TAU_PULSE_MS = 23.61
STARFIRE_FREQ = PHI ** 2          # ≈ 2.618034

# 144…2584 (Fibonacci) ↔ OBSERVATION…TRANSCENDENCE
...
def verify_entry(e: dict) -> EntryResult:
    ...
    # φ-jitter soak — derive θ from digest, correct, label
...
def _smoke() -> int:
    """Self-contained smoke test for the jitter path."""
    print("🌌 jitter soak smoke")
    print(f"   φ           = {PHI:.15f}")
    print(f"   γ_jitter    = {GAMMA_JITTER:.10f}")
    print(f"   starfire φ² = {STARFIRE_FREQ:.10f}")
    trace = soak_trace(steps=8)
    for row in trace:
        print(f"   t={row['t']:>6.3f}  θ={row['theta']:.8f}  corr={row['corrected']:.8f}")
    sample = apply_jitter_correction(DELTA_THETA, 0)
    print(f"   apply_jitter_correction(δθ, 0) = {sample:.10f}")
    print(f"   ninja(1500) = {_ninja_label(1500)}")
    print(f"   ninja(144)  = {_ninja_label(144)}")
    print(f"   ninja(9999) = {_ninja_label(9999)}")
    return 0
...
def main() -> int:
    ...
    immutable_violations = [r.path for r in results
    ...
    merged = {
        ...
        "hash_algo": HASH_ALGO,
        "jitter": {
            "enabled": bool(args.jitter),
            "gamma_jitter": GAMMA_JITTER,
            "tau_pulse_ms": TAU_PULSE_MS,
            "delta_theta": DELTA_THETA,
            "t_max_soak": T_MAX_SOAK,
            "starfire_freq": STARFIRE_FREQ,
    ...
    print(f"   φ-jitter : γ={GAMMA_JITTER:.6f}  starfire={STARFIRE_FREQ:.6f}"
          f"  soak_steps={args.soak_steps}")
    print(f"   written  : {out_yaml}, {out_json}")

    hard_fail = bool(conflicts) or bool(immutable_violations)
