...
def trimer_eigen(a=None):
    if a is None:
        a = 1 / PHI
    H = np.array([[1, a, 0], [a, 2, a], [0, a, 1]], dtype=float)
...
def make_H(a):
    return np.array([[1, a, 0], [a, 2, a], [0, a, 1]], dtype=float)
...
def route(x, W=None, b=None, tool_thresh=0.6, agent_thresh=0.4):
    ...
    return {"probs": dict(zip(D_SPACE, probs)), "decision": decision, "confidence": p}
...
# ── 5. ADAPTIVE FEEDBACK ─────────────────
def adapt_hamiltonian(a, overlap, target=0.5, lr=0.01, a_min=PRECISION_FLOOR, a_max=2.0):
...
if __name__ == "__main__":
    ...
    Ls = [
        np.sqrt(0.05) * np.array([[0, 1, 0], [0, 0, 0], [0, 0, 0]], dtype=complex)
    ]
    ...
    for i, seed in enumerate([0, 1, 99]):
        ...
        print(
            f"step={i + 1}  gate={res['coherence_gate']}  "
